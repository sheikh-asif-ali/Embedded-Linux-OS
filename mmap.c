// C rogram using mmap() and munmap()
...
void write_mmap_sample_data();	//function decleration
...
int main()
{
	...
	int fd;
	int unmapstatus;
	write_mmap_sample_data();	//function call
	...
	//data = mmap(0, mmapstat.st_size, PROT_WRITE, MAP_PRIVATE, fd, 0);
...
void write_mmap_sample_data()	//function defination
{
	int fd;
	char ch;
